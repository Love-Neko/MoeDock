...
## 🤖 按宿主能力选择的实战提示词模板 (Platform-Specific Prompts)

### 1. 对话式图像生成平台（防扩写 & 强制纯白底）
> 如图像模型会自动扩写提示词，先加上强制约束声明：
```text
...
### 2. 提示词模型（辅助生成目标平台命令）
> 若当前模型不直接生成图像，可生成可粘贴到目标图像平台的标准命令：
```text
...
### 3. 支持本地执行的图像工具
> 将长句英文描述传入当前宿主提供的图像工具，并设置 1:1 画幅（若工具支持）：
```text
